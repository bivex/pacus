...
def rel(from_file: Path, to_file: Path) -> str:
    """Absolute path from server root (out_dir)."""
    # Convert to absolute path string without out_dir prefix
    path_str = str(to_file).replace("\\", "/")
    out_dir_str = str(out_dir).replace("\\", "/")
    if path_str.startswith(out_dir_str):
        return "/" + path_str[len(out_dir_str) :].lstrip("/")
